...
class Actor:
    ...
    def build_NN(self):
        ...
        # add dense layers
        net = layers.Dense(units=16, activation=None)(states)
...
class Task2_Policy(BaseAgent):
    def __init__(self, task):
        ...
        print ("task observation space low:\n{0}".format(self.task.observation_space.low))
        '''
        self.state_low = np.concatenate(self.task.observation_space.low[0:3],
                                        self.task.observation_space.low[7:10])

        self.state_high = np.concatenate(self.task.observation_space.high[0:3],
                                        self.task.observation_space.high[7:10])
        '''
    ...
    def preprocess_state(self, state):
        ...
        return np.array([state[0], state[1], state[2], state[7], state[8], state[9]])  # position only
    ...
    def step(self, state, reward, done):
        ...
        # scale to [0.0, 1.0]
        state[0:3] = (state[0:3]-self.state_low)/3
...
class OUNoise:
    ...
    def __init__(self, size, mu=None, theta=0.15, sigma=1.0):
